...
const HubSpotIntegration: React.FC<HubSpotIntegrationProps> = ({ orgId, onBack }) => {
  const router = useRouter();
  const searchParams = useSearchParams();
  const [status, setStatus] = useState<HubSpotStatus | null>(null);
  const [loading, setLoading] = useState(false);
  ...
  const fetchStatus = async () => {
    ...
    try {
      setLoading(true);
      const res = await axios.get(`http://localhost:5000/api/v1/tools/hubspot/status/${orgId}`, {
  ...
  const handleDisconnect = async () => {
    ...
    try {
      setLoading(true);
      await axios.delete(`http://localhost:5000/api/v1/tools/hubspot/disconnect/${orgId}`, {
        headers: { Authorization: token },
      });
      toast.success("HubSpot disconnected successfully");
      setStatus(null);
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-md">
      ...
      <h2 className="text-2xl font-bold mb-6 text-gray-800">HubSpot Integration</h2>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {status?.isConnected ? (
            <div className="space-y-4">
              <div className="p-4 bg-green-100 rounded-md">
                <p className="text-green-800 font-medium">Connected to HubSpot</p>
                {status.lastSyncedAt && (
                  <p className="text-sm text-gray-600">
                    Last synced: {new Date(status.lastSyncedAt).toLocaleString()}
                  </p>
            ...
            <div className="space-y-4">
              <p className="text-gray-600">Connect your HubSpot to sync leads and Q&A pairs automatically.</p>
              <HubSpotConnectButton />
            </div>
          )}
        </div>
